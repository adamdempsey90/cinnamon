    ...
    for(k=-NGHX3;k<nx3+NGHX3;k++) {
		for(j=-NGHX2;j<nx2+NGHX2;j++) {
			for(i=-NGHX1;i<nx1+NGHX1;i++) {
				indx = INDEX(i,j,k);
				wave = sin(2*M_PI*xc1[i]/params->l);
				cs2 = params->gamma * params->p0/params->d0;


				rho[indx] = params->d0 + params->amp_l * wave
										+ params->amp_r * wave
										+ params->amp_c * wave;
				u1 = params->u0 + sqrt(cs2)/params->d0 *(params->amp_r - params->amp_l)*wave;
				u2 = params->v0 + params->amp_s * wave;
				pres = params->p0 + cs2 *(params->amp_r + params->amp_l)*wave;
